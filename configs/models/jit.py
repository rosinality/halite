...
from halite.transformers.embedding import PatchEmbedding
...
def jit(
    ...
    patch_embedding = PatchEmbedding(
        ...
        patch_size,
        use_bias=False,
        patch_embedding_init=partial(nn.init.xavier_uniform_),
    )
    time_embedding = TimeEmbedding(dim)
    label_embedding = LabelEmbedding(n_labels, dim)
    post_embedding = BottleneckEmbedding(
        patch_dim, dim, linear_init=partial(nn.init.xavier_uniform_)
    )
    # patch_embedding = BottleneckPatchEmbed(256, 16, 3, patch_dim, dim)
    # post_embedding = None

    grid_size = image_size // patch_size
    n_patches = grid_size**2

    pos_embed = SinusoidalEmbedding(dim, grid_size, n_patches)
    rope_dim = head_dim // 2
    pos_rope = VisionRoPE(rope_dim, grid_size)
    ...
    for i in range(n_layers):
        ...
        # blocks += [JiTBlock(dim, n_heads)]

    transformer = JiT(
        embedding=patch_embedding,
        time_embedding=time_embedding,
        label_embedding=label_embedding,
        post_embed=post_embedding,
        pos_embed=pos_embed,
        pos_rope=pos_rope,
