...
class RMSNorm(nn.Module):
    ...
    def forward(self, input):
        if self.fast:
            return fused_rms_norm_fn(input, self.weight, self.eps)

        input_fp32 = input.to(torch.float32)
        norm = input_fp32 * torch.rsqrt(
            input_fp32.pow(2).mean(-1, keepdim=True) + self.eps
        )

        if self.elementwise_affine:
            out = self.weight * norm.type_as(input)
        else:
            out = norm.type_as(input)
