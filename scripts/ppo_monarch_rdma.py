...
class Trainer(Actor):
    ...
    async def initialize(
        ...
        os.environ["MASTER_PORT"] = str(local_master_port)
        ...
        self.global_pg = stateless_init_process_group(
            master_addr, global_master_port, self.rank, world_size, self.device_id
        )
...
async def main():
    ...
    generator_mesh = this_host().spawn_procs(
        per_host={"gpus": conf.monarch.generator_mesh_size}
    )

    trainer_mesh = this_host().spawn_procs(
        per_host={"gpus": conf.monarch.trainer_mesh_size}
    )

    await trainer_mesh.logging_option(True, None)
    await generator_mesh.logging_option(True, None)

    environment_mesh = this_host().spawn_procs(
        {"gpus": conf.monarch.generator_mesh_size}
    )
    replay_buffers_mesh = this_host().spawn_procs(
        {"gpus": conf.monarch.trainer_mesh_size}
    )
    trajectory_mesh = this_host().spawn_procs({"gpus": 1})
    environment_queue_mesh = this_host().spawn_procs({"gpus": 1})
    rollout_mesh = this_host().spawn_procs({"gpus": 1})

    replay_buffers = replay_buffers_mesh.spawn(
        "replay_buffers",
        BasicReplayBuffer,
        conf.training.train_batch_size // conf.monarch.trainer_mesh_size,
    )
    trajectory_queue = trajectory_mesh.spawn("trajectory_queue", BidirectionalQueue)
    environment_queue = environment_queue_mesh.spawn(
        "environment_queue", BidirectionalQueue
    )
    generator = generator_mesh.spawn("generator", Generator, conf, trajectory_queue)
    rollout_manager = rollout_mesh.spawn(
        "rollout",
        RolloutManager,
        conf,
        trajectory_queue,
    ...
    environment = environment_mesh.spawn(
        "environment",
        Environment,
        conf,
        environment_queue,
    )
    trainer = trainer_mesh.spawn("trainer", Trainer, conf, replay_buffers, generator)
